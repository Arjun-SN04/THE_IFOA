...
import { isFieldVisible } from './formSchema.js';

function SummaryRow({ label, value, fullWidth = false, uppercaseLabels = true }) {
  ...
  return (
    <div className={`rounded-xl border border-slate-100 bg-slate-50/50 p-3.5 ${fullWidth ? 'sm:col-span-2' : ''}`}>
      <div className={`text-[10px] font-bold tracking-wider text-slate-400 mb-1 ${uppercaseLabels ? 'uppercase' : ''}`}>{label}</div>
...
export function SubmissionSummary({ schema, answers, uppercaseLabels = true }) {
